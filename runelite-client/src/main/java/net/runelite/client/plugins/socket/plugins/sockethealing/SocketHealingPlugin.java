...
public class SocketHealingPlugin extends Plugin
{
	...
	{
		this.overlayManager.add(socketHealingOverlay);
		this.lastRefresh = 0;
	...
	{
		this.overlayManager.remove(socketHealingOverlay);
	}
	...
	private void onGameTick(GameTick event)
	{
		if (this.client.getGameState() == GameState.LOGGED_IN)
		{
			...
			int hpLevel = this.client.getRealSkillLevel(Skill.HITPOINTS);

			SocketHealingPlayer playerHealth;

			synchronized (this.partyMembers)
			{
				playerHealth = (SocketHealingPlayer)this.partyMembers.get(name);
	...
	public void onSocketReceivePacket(SocketReceivePacket event)
	{
		try
		{
			...
			if (payload.has("player-health"))
			{
				...
				synchronized (this.partyMembers)
				{
					SocketHealingPlayer playerHealth = (SocketHealingPlayer)this.partyMembers.get(targetName);
					if (playerHealth == null)
					{
